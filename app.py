	...
	if data["object"] == "page":
		for entry in data["entry"]:
			for messaging_event in entry["messaging"]:
				...
				user_state = state_coll.find_one({"user_id": sender_id})
				handle_message(messaging_event, user_state)

	return "ok", 200


def handle_message(messaging_event, user_state):
	sender_id = messaging_event["sender"]["id"]
	recipient_id = messaging_event["recipient"]["id"]
	state_id, state_map = None, None

	if user_state is not None:
		user_state_id = user_state["_id"]
		user_state_map = user_state["map"]

	if messaging_event.get("postback"):
		handle_postback_message(messaging_event, user_state)
	elif messaging_event.get("message"):
		handle_regular_message(messaging_event, user_state)
...
def handle_regular_message(messaging_event, state):
	# arbitrary message has been received
	message_text = messaging_event["message"]["text"]

	# check to see if user exists in database
	user = user_coll.find_one({"user_id": sender_id})
	...
	if user is None or not user["is_onboarded"]:
		...
		goal_user = goal_coll.find_one({"user_id": sender_id})

		if goal_user is None:
		...
		# onboard user
		if user_state_map is None or not state_map["goal_title"]["is_message_sent"]:
	...
	if state_map["expense"]["flow_instantiated"]:
		# puserumably last stage of expense specification
		...
		# we remove the current balance refuserh for now as it is failing
